...
export class PokemonListComponent implements OnInit {
  ...
  getPokemonList() {
    if (
      ...
      this.pokemonList.length === this.pokemonCount
    ) {
      return;
    }
  ...
  processPokemonData(resp: any) {
    const details = resp.results.map((pokemon: any) =>
      this.pokemonService.getPokemonData("pokemon/" + pokemon.name),
    );
    concat(...details)
      .pipe(takeUntil(this.unsubscribe))
  ...
  showPokemonDetails(pokemon: any) {
    ...
    this.router.navigate(
      [{ outlets: { modal: ["home", pokemon.id, this.pokemonCount] } }],
